...
def add():
    ...
    with open('temp_file.csv', 'a', newline="") as fc:
        ...
        if os.stat('temp_file.csv').st_size == 0:
            with open('temp_file.csv', 'a',) as fc:
                fc.write(
                    f'Bill NO : {bill_no(current_bill_no)}\nDate : {date}\nFauji Ration Store\nGST no : 02AGPPC1727P1ZB\nP.no : 7018423400 \nTo : {to_name}\n')
    ...
    total = total+total_prize
    total_label = ttk.Label(win, text=gst_5 + round(gst_12,2) + round(gst_18,2) + round(gst_28,2) + round(total,2))
    total_label.grid(row=4, column=5, padx=20)

    l1 = ttk.Label(win, text=prod_name)
    l1.grid(row=counter, column=0, sticky=W)

    l2 = ttk.Label(win, text=total_prize)
...
def save_copy():
    ...
    if os.path.exists(r"temp_file.csv"):
        if os.path.exists("fauji ration store"):
            if os.path.exists(f'fauji ration store\Fauji {date}({name_count}).csv'):
                d_call()
                with open('temp_file.csv', 'r') as fc:
                    with open(f'fauji ration store\Fauji {date}({name_count}).csv', 'w') as wf:
                        wf.write(fc.read())
                with open(f'fauji ration store\Fauji {date}({name_count}).csv', 'a') as wf:
                    wf.write(
